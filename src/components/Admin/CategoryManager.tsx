...
export default function CategoryManager({ categories, onAdd, onEdit, onDelete }: Props) {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('確定要刪除此分類？儲存於此分類的文章將被移至預設分類。')) {
      setSubmitting(true);
      setLocalError('');
      try {
        await onDelete(id);
    ...
    <GlassCard className="block mb-8 !p-0">
      <div className="p-6 sm:p-8 md:p-10">
        ...
        <form onSubmit={handleAdd} className="space-y-4 mb-8">
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded font-semibold tracking-wide shadow hover:scale-105 transition-all"
            disabled={submitting}
          >
            {submitting ? '新增中...' : '新增分類'}
          </button>
                ...
                <tr key={cat.id} className="hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-right">
                    <div className="inline-flex gap-2">
                      {editingId === cat.id ? (
                        <>
                          <button className="px-3 py-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded shadow font-semibold" onClick={() => handleEdit(cat.id)} disabled={submitting}>儲存</button>
                          <button className="px-3 py-1 bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded shadow font-semibold" onClick={() => { setEditingId(null); setEditingName(''); }} disabled={submitting}>取消</button>
                        </>
                      ) : (
                        <>
                          <button className="px-3 py-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded shadow font-semibold" onClick={() => { setEditingId(cat.id); setEditingName(cat.name); }} disabled={submitting}>編輯</button>
                          <button className="px-3 py-1 bg-red-500 text-white rounded shadow hover:bg-red-600 font-semibold" onClick={() => handleDelete(cat.id)} disabled={submitting}>刪除</button>
                        </>
                      )}
                    </div>
                  </td>
